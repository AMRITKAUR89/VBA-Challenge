# VBA-Challenge
This code was written to get the desired outcome for the VBA Challenge in Module 2. <b> (Final Full code at the end )</b>
## Declarations
...
<pre>Sub ModTwoSolution()
    ...
    highestDiffString = ""
    lowestDiff = 1E+308 ' Very high initial value
    lowestDiffString = ""
    highestvol = -1E+308 ' Very low initial value
    highestvolString = ""

    ' Loop through each worksheet
    For Each ws In ThisWorkbook.Worksheets
    
        ' Convert date strings in column B
        ...
        For i = 2 To lastRow
            ...
            If Len(originalDate) = 8 And IsNumeric(originalDate) Then
                ...
                convertedDate = DateSerial(yearPart, monthPart, dayPart) ' Corrected day and month parts
                ws.Cells(i, 2).Value = Format(convertedDate, "mm/dd/yyyy")
            ElseIf IsDate(originalDate) Then
                Dim dateParts() As String
        ...
        If uniqueStrings.Count > 0 Then
            ReDim sortedStrings(1 To uniqueStrings.Count)
            For i = 1 To uniqueStrings.Count
                sortedStrings(i) = uniqueStrings(i)
            Next i
            QuickSort sortedStrings, LBound(sortedStrings), UBound(sortedStrings) ' Call the sorting function
            ...
            For i = LBound(sortedStrings) To UBound(sortedStrings)
                ...
                ' Sum total stock volume and find opening/closing values for the ticker
                For Each searchCell In searchRange
                    If searchCell.Value = sortedStrings(i) Then
                        sumG = sumG + ws.Cells(searchCell.Row, "G").Value
                        
                        ' Find the first and last occurrence in column C and F
